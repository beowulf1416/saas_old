...
class ItemsStore(BaseStore):
    ...
    def add(self, item: dict):
        ...
        '''
        try:
            super(ItemsStore, self).runProcTransactional('inventory.item_add', [
                item['clientId'],
                item['itemId'],
                item['name'],
                item['description'],
    ...
    def update(self, item: dict):
        ...
        '''
        try:
            super(ItemsStore, self).runProcTransactional('inventory.item_update', [
                item['clientId'],
                item['itemId'],
                item['name'],
                item['description'],
    ...
    def get(self, itemId: UUID):
        try:
            [result, ] = super(ItemsStore, self).runProc('inventory.item_by_id', [itemId, ])
            return result
        except Exception as e:
            log.error(e)
            raise Exception('Unable to retrieve inventory item')
